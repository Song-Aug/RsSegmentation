...
def train_one_epoch(model, train_loader, criterion, optimizer, device, metrics, epoch):
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        with torch.no_grad():
            ...
            pred_binary = (pred1 > 0.5).float().squeeze(1).cpu().numpy().astype(np.int64)
...
def main():
    ...
    try:
        ...
        for epoch in range(config['num_epochs']):       
            ...
            swanlab.log({
                'train/loss': train_loss, 'train/dice': train_dice, 'train/iou_custom': train_iou,
                'train/iou': train_result['iou'], 'train/precision': train_result['precision'],
                'train/recall': train_result['recall'], 'train/f1': train_result['f1'],
                'val/loss': val_loss, 'val/dice': val_dice, 'val/iou_custom': val_iou,
                'val/iou': val_result['iou'], 'val/precision': val_result['precision'],
                'val/recall': val_result['recall'], 'val/f1': val_result['f1']
            })
        ...
        swanlab.log({
            'test/dice': test_dice, 'test/iou_custom': test_iou,
            'test/iou': test_result['iou'], 'test/precision': test_result['precision'],
            'test/recall': test_result['recall'], 'test/f1': test_result['f1']
        })
