...
class GCFM(nn.Module):
    def __init__(self, in_channels, out_channels, k):
        ...
        self.fusion_conv = nn.Sequential(
            nn.Conv2d(self.num_channels, out_channels, kernel_size=3, padding=1, bias=False),
    ...
    def forward(self, x):
        ...
        # As per equation (3)
        x_up = F.interpolate(x, scale_factor=self.k, mode='bilinear', align_corners=True)
        value = self.conv3(x_up).view(x.size(0), self.num_channels, -1)
        
        fused_features = torch.bmm(correlation_matrix, value)
        fused_features = fused_features.view(x.size(0), self.num_channels, x_up.size(2), x_up.size(3))
...
class GlobalStream(nn.Module):
    ...
    def forward(self, features):
        ...
        g2_up = F.interpolate(g2, size=g4.shape[2:], mode='bilinear', align_corners=True)
        g3_up = F.interpolate(g3, size=g4.shape[2:], mode='bilinear', align_corners=True)
...
class ShapeStream(nn.Module):
    ...
    def forward(self, features):
        ...
        s1 = self.conv_s1(e1)
        s2 = F.interpolate(self.conv_s2(e2), size=target_size, mode='bilinear', align_corners=True)
        s3 = F.interpolate(self.conv_s3(e3), size=target_size, mode='bilinear', align_corners=True)
        s4 = F.interpolate(self.conv_s4(e4), size=target_size, mode='bilinear', align_corners=True)
...
class STDSNet(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.encoder.layers):
            x_out = layer(x)
            # If x_out is a tuple, take the first element. Otherwise, it's just the tensor.
            x = x_out[0] if isinstance(x_out, tuple) else x_out
        ...
        global_pred = F.interpolate(global_pred, size=input_size, mode='bilinear', align_corners=True)
        shape_pred = F.interpolate(shape_pred, size=input_size, mode='bilinear', align_corners=True)
