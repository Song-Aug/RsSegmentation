    ...
    return A.Compose(
        [
            ...
            A.HueSaturationValue(
                p=0.3, hue_shift_limit=20, sat_shift_limit=30, val_shift_limit=20
            ),
            A.GaussNoise(p=0.2),
            A.OneOf(
                [
...
def create_dataloaders(
    root_dir, batch_size=16, num_workers=4, image_size=512, augment=True, use_nir=True
):
    train_transform = (
        get_train_augmentations(image_size, use_nir)
        if augment
        else get_val_augmentations(image_size, use_nir)
    )
