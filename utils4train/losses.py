...
class DiceLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        union = torch.sum(probs, dim=(0, 2, 3)) + torch.sum(
            targets_onehot, dim=(0, 2, 3)
        )
...
def stdsnet_loss(
    ...
    """Composite loss for STDSNet combining CE and Dice for both branches."""
...
def hdnet_loss(outputs, labels, weights=None):
    ...
    x_seg, x_bd, seg1, seg2, seg3, seg4, seg5, seg6, bd1, bd2, bd3, bd4, bd5, bd6 = (
        outputs
    )
...
def mssdmpanet_y_bce_loss(pred1, pred2, pred3, pred4, pred5, y):
    ...
    loss1 = bce_with_logits(
        pred1, F.interpolate(y, size=pred1.shape[2:], mode="nearest")
    )
    loss2 = bce_with_logits(
        pred2, F.interpolate(y, size=pred2.shape[2:], mode="nearest")
    )
    loss3 = bce_with_logits(
        pred3, F.interpolate(y, size=pred3.shape[2:], mode="nearest")
    )
    loss4 = bce_with_logits(
        pred4, F.interpolate(y, size=pred4.shape[2:], mode="nearest")
    )
    loss5 = bce_with_logits(
        pred5, F.interpolate(y, size=pred5.shape[2:], mode="nearest")
    )
