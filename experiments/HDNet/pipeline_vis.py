...
def load_model(model_path, device, config):
    """加载模型"""
    model = HDNet(
        base_channel=config.get("base_channel", 48),
...
def inference_vis_dataset(
    ...
    with torch.no_grad():
        for img_file in tqdm(image_files, desc="推理中"):
            ...
            # HDNet 输出: x_seg, x_bd, seg1, seg2, seg3, seg4, seg5, seg6, bd1, bd2, bd3, bd4, bd5, bd6
            # 取主分割输出
...
def main():
    ...
    parser = argparse.ArgumentParser(description="HDNet 可视化推理脚本")
    ...
    if args.output:
        ...
        output_dir = os.path.join("./vis_results", config.get("model_name", "HDNet"))
