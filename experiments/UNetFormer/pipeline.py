...
from loss_function import unetformer_loss
...
def train_one_epoch(model, train_loader, optimizer, device, metrics, epoch, scaler):
    ...
    model.train()
    metrics.reset()
    loss_meter = {"total": 0.0, "seg": 0.0, "aux": 0.0}
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        with autocast(device_type='cuda', dtype=torch.float16):
            ...
            total_loss_batch, seg_loss_batch, aux_loss_batch = unetformer_loss(
                outputs, labels,
                aux_weight=config.get("aux_loss_weight", 0.4),
            )
...
def main():
    ...
    try:
        ...
        model = UNetFormer(
            decode_channels=64,
            dropout=0.1,
            backbone_name=config.get("backbone", "swsl_resnet18"),
