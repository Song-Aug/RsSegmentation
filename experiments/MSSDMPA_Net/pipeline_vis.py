...
def load_model(model_path, device, config):
    ...
    model = MSSDMPA_Net(
        ...
        num_classes=config.get("num_classes", 1),  # MSSDMPA-Net 默认使用单通道输出
    )
...
def inference_vis_dataset(
    ...
    with torch.no_grad():
        for img_file in tqdm(image_files, desc="推理中"):
            ...
            # MSSDMPA_Net 输出: x_out, x1_out, x2_out, x3_out, x4_out
            # 取主输出
            main_output = outputs[0] if isinstance(outputs, (list, tuple)) else outputs

            # MSSDMPA-Net 使用单通道输出 (num_classes=1)
            # 使用 sigmoid 激活函数
            pred = torch.sigmoid(main_output)[0, 0].cpu().numpy()
...
def main():
    ...
    parser = argparse.ArgumentParser(description="MSSDMPA-Net 可视化推理脚本")
    ...
    if args.output:
        ...
        output_dir = os.path.join("./vis_results", config.get("model_name", "MSSDMPA_Net"))
