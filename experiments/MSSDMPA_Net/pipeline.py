...
from model import MSSDMPA_Net
from dataprocess import get_loaders
from loss_function import mssdmpanet_loss, MSSDMPA_IoU
...
def train_one_epoch(model, train_loader, optimizer, device, metrics, epoch, scaler):
    ...
    model.train()
    metrics.reset()
    loss_meter = {"total": 0.0, "main": 0.0, "aux": 0.0}
    iou_calculator = MSSDMPA_IoU()
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        with autocast(device_type='cuda', dtype=torch.float16):
            ...
            total_loss_batch, main_loss_batch, aux_loss_batch = mssdmpanet_loss(outputs, labels)
        ...
        loss_meter["main"] += main_loss_batch.item()
        loss_meter["aux"] += aux_loss_batch.item()

        # 计算指标 (使用最高分辨率输出)
        pred5 = outputs[4]  # pred5 是最高分辨率输出
        pred_binary = (torch.sigmoid(pred5) > 0.5).long().squeeze(1)
        targets = labels.cpu().numpy()
        metrics.update(pred_binary.cpu().numpy(), targets)

        pbar.set_postfix({
            "Total": f"{total_loss_batch.item():.4f}",
            "Main": f"{main_loss_batch.item():.4f}",
...
def main():
    ...
    try:
        ...
        # 评估指标 (MSSDMPA-Net 使用 2 类：背景和建筑物)
        train_metrics = SegmentationMetrics(num_classes=2)
        val_metrics = SegmentationMetrics(num_classes=2)
        test_metrics = SegmentationMetrics(num_classes=2)
