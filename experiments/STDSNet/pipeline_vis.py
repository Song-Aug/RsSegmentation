...
def inference_vis_dataset(
    ...
    with torch.no_grad():
        for img_file in tqdm(image_files, desc="推理中"):
            ...
            # STDSNet 输出: global_pred, shape_pred
            # 使用 global_pred 作为主输出
            if isinstance(outputs, (list, tuple)):
                main_output = outputs[0]  # global_pred
            else:
                main_output = outputs
...
def main():
    ...
    parser = argparse.ArgumentParser(description="STDSNet 可视化推理脚本")
    ...
    if args.output:
        ...
        output_dir = os.path.join("./vis_results", config.get("model_name", "STDSNet"))
